...
from src.search_operations import operations_search
...
def main():
    ...
    while True:
        if user_dialogue == "1":
            ...
            transactions = get_operations_info(path_to_file)
            break
        elif user_dialogue == "2":
            print("Для обработки выбран CSV-файл")
            transactions = transactions_csv(join_path_csv)
            break
        elif user_dialogue == "3":
            print("Для обработки выбран XLSX-файл")
            transactions = transactions_excel(join_path_exc)
            break
        else:
            user_dialogue = input(
    ...
    while True:
        if (
            ...
            or state_operations == "CANCELED"
            or state_operations == "PENDING"
        ):
    ...
    sort_currency = input(
        "Выводить только рублевые тразакции?\nВведите да или нет\nВвод: "
    ).lower()
    ...
    while True:
        if descriptions_filter == "да":
            ...
            final_filter_operations = operations_search(
                sort_currency_user, search_string_user
            )
            break
